...
import { anonymizeImports, anonymizeType } from './anonymize.js';
...
const generateDescriptors = (originalMetadata, lookupFn, checksumToIdx, typesBuilder, checksumBuilder, key, paths, genesis) => {
  const prefix = capitalize(key);
  const palletInteractions = (mapFn) => Object.fromEntries(
    originalMetadata.pallets.map((pallet) => [pallet.name, mapFn(pallet) ?? []]).filter((v) => v[1].length)
  );
  const mapEnumInteractions = (typeId) => {
  ...
  const allInteractions = {
    storage: palletInteractions(
      (pallet) => pallet.storage?.items.map((it) => `'${it.name}'`)
    ),
    tx: palletInteractions((pallet) => mapEnumInteractions(pallet.calls?.type)),
    events: palletInteractions(
      (pallet) => mapEnumInteractions(pallet.events?.type)
    ),
    errors: palletInteractions(
      (pallet) => mapEnumInteractions(pallet.errors?.type)
    ),
    constants: palletInteractions(
      (pallet) => pallet.constants.map((v) => `'${v.name}'`)
    ),
    viewFns: palletInteractions(
      (pallet) => pallet.viewFns.map((v) => `'${v.name}'`)
    ),
...
const knownSignedExtensions = /* @__PURE__ */ new Set([
  "CheckNonce",
  "CheckMortality",
...
function getExtensionsType(lookup, typesBuilder) {
  const result = {};
  ...
  return `{
  ${Object.entries(result).map(([id, type]) => `"${id}": ${type}`).join(",\n")}
  }`;
}
...
//# sourceMappingURL=generate-descriptors.js.map

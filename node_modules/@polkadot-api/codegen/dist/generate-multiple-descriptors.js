...
import { generateDescriptors, capitalize } from './generate-descriptors.js';
import { generateTypes } from './generate-types.js';
import { getUsedTypes } from './get-used-types.js';
import { knownTypes } from './known-types.js';
import { getTypesBuilder, defaultDeclarations } from './types-builder.js';
import { applyWhitelist } from './whitelist.js';

const generateMultipleDescriptors = (chains, paths, options = {}) => {
  const globalWhitelist = options.whitelist ? Array.isArray(options.whitelist) ? options.whitelist : options.whitelist["*"] ?? [] : null;
  const chainData = chains.map((chain) => {
    const chainWhitelist = options.whitelist && !Array.isArray(options.whitelist) ? options.whitelist[chain.key] ?? null : null;
    const whitelist = globalWhitelist || chainWhitelist ? [...globalWhitelist ?? [], ...chainWhitelist ?? []] : null;
    const metadata = options.whitelist ? applyWhitelist(chain.metadata, whitelist) : chain.metadata;
...
//# sourceMappingURL=generate-multiple-descriptors.js.map

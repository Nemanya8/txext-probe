...
import { getInkDynamicBuilder } from './dynamic-builders.js';
import { getInkLookup } from './get-lookup.js';

const createInkClient = (inkContract) => {
...
const inkClientCache = /* @__PURE__ */ new WeakMap();
...
const buildCallable = (builder) => (label) => {
  ...
  return {
    encode: (value) => codecs.call.enc(value || {}),
    decode: (response) => codecs.value.dec(response.data)
  };
};
const buildStorage = (builder) => (label) => {
  const codecs = builder(label);
  return {
    encode: (key) => codecs.key.enc(key),
    decode: (response) => codecs.value.dec(response)
  };
};
const buildEventV4 = (eventsDecoder) => {
  const decode = (value) => {
    return eventsDecoder().dec(value.data);
  };
...
const buildEventV5 = (lookup, eventDecoder) => {
  ...
  const decode = (value, signatureTopic) => {
    if (signatureTopic != null) {
      ...
      return eventDecoder(signatureTopic).dec(value.data);
    }
    if (!hasAnonymousEvents) {
      throw new Error("Event signature topic required");
    }
    return eventDecoder(void 0).dec(value.data);
  };
  const filter = (address, events = []) => {
    const addrEq = (a) => (typeof a === "string" ? a : toHex(a)) === address;
...
export { getInkClient };
//# sourceMappingURL=ink-client.js.map

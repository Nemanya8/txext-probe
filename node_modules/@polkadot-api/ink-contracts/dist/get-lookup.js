...
import { pjsTypes } from './metadata-pjs-types.js';
...
function getStorageLayout(metadata, lookup) {
  ...
  const readLayout = (node, path = []) => {
    ...
    if ("root" in node) {
      const keyPrefix = Number(metadata.version) === 4 ? toHex(Binary.fromHex(node.root.root_key).reverse()) : node.root.root_key;

...
import { fromAbortControllerFn } from '../chainHead/enhancers/fromAbortControllerFn.js';
...
const getArchive = ({ storageSubscription, ...archive }) => (getRuntime) => {
  ...
  const call$ = (hash, fnName, callParameters) => fromAbortControllerFn(archive.call)(hash, fnName, callParameters).pipe(
    map(fromHex)
  );
  const rawHeader$ = fromAbortControllerFn(archive.header);
  const body$ = (hash) => fromAbortControllerFn(archive.body)(hash).pipe(map((v) => v.map(fromHex)));

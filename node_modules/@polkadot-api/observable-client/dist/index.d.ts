...
import { UnifiedMetadata, SizedHex, HexString, Decoder, Codec, SS58String, ResultPayload } from '@polkadot-api/substrate-bindings';
import { VoidVar, TupleVar, StructVar, ArrayVar, LookupEntry, MetadataLookup, getDynamicBuilder } from '@polkadot-api/metadata-builders';

type Hasher = (input: Uint8Array) => Uint8Array;

type EnumEntry = (VoidVar | TupleVar | StructVar | ArrayVar | {
    type: "lookupEntry";
    value: LookupEntry;
}) & {
    idx: number;
...
type MetadataMaps = {
    pallets: Record<string, {
        storage: Map<string, NonNullable<UnifiedMetadata["pallets"][number]["storage"]>["items"][number]>;
        event: Map<string, EnumEntry>;
        error: Map<string, EnumEntry>;
        call: Map<string, EnumEntry>;
        const: Map<string, NonNullable<UnifiedMetadata["pallets"][number]["constants"]>[number]>;
        view: Map<string, NonNullable<UnifiedMetadata["pallets"][number]["viewFns"]>[number]>;
    }>;
    api: Record<string, Map<string, NonNullable<UnifiedMetadata["apis"][number]["methods"][number]>>>;
    extensions: Record<number, Record<string, {
        type: number;
...
declare const enum PinnedBlockState {
    Initializing = 0,// We are waiting to get ready on the "first" best-block event of the current or next chainHead follow subscription.
    Ready = 1,// Downstream events can be derived only when the PinnedBlockState is in this state
    RecoveringInit = 2,// There has been a stop-event and we are hopping that the first initialized event has an overlap with the previous blocks.
    RecoveringFin = 3
}
type RecoveringInitState = {
    type: PinnedBlockState.RecoveringInit;
};
type ReadyState = {
    type: PinnedBlockState.Ready;
};
...
type PinnedBlocks = {
    ...
    state: RecoveringInitState | RecoveringFinState | ReadyState | InitializingState;
};
...
declare const getChainHead$: (chainHead: ChainHead, getCachedMetadata: (codeHash: string) => Observable<Uint8Array | null>, setCachedMetadata: (codeHash: string, metadataRaw: Uint8Array) => void) => {
    follow$: Observable<(_polkadot_api_substrate_client.BestBlockChanged | _polkadot_api_substrate_client.Finalized | (_polkadot_api_substrate_client.Initialized & {
        number: number;
        parentHash: string;
        runtimeChanges: Map<string, string>;
        hasNewRuntime: boolean;
    }) | (_polkadot_api_substrate_client.NewBlock & {
        newRuntime: _polkadot_api_substrate_client.Runtime | null;
    } & {
        codeHash?: string;
    })) | {
        type: "stop-error";
    ...
    metadata$: Observable<{
        ...
        extrinsic: {
            version: number[];
            signedExtensions: Record<number, Array<{
                identifier: string;
                type: number;
                additionalSigned: number;
            }>>;
    ...
    body$: (hash: string) => Observable<Uint8Array<ArrayBufferLike>[]>;
    call$: (hash: string | null, fn: string, args: string) => Observable<Uint8Array<ArrayBufferLike>>;
    storage$: <Type extends StorageItemInput["type"], M extends undefined | ((data: StorageResult<Type>, ctx: RuntimeContext) => any)>(hash: string | null, type: Type, keyMapper: (ctx: RuntimeContext) => string, childTrie?: string | null | undefined, mapper?: M | undefined) => Observable<{
        block: BlockInfo;
        value: undefined extends M ? StorageResult<Type> : ReturnType<NonNullable<M>>;
    }>;
    ...
    trackTx$: (tx: Uint8Array) => Observable<AnalyzedBlock>;
    trackTxWithoutEvents$: (tx: Uint8Array) => Observable<AnalyzedBlock>;
    validateTx$: (blockHash: HexString, tx: Uint8Array) => Observable<_polkadot_api_substrate_bindings.ResultPayload<any, any>>;
...
declare const getArchive: ({ storageSubscription, ...archive }: Archive) => (getRuntime: (codeHash: string) => Observable<RuntimeContext | null>) => {
    ...
    call$: (hash: string, fnName: string, callParameters: string) => Observable<Uint8Array<ArrayBufferLike>>;
...
export type { AnalyzedBlock, Archive$, BlockInfo, ChainHead$, EnumEntry, MetadataMaps, ObservableClient, PinnedBlock, PinnedBlocks, RuntimeContext, SystemEvent };

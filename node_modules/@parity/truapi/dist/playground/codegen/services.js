export const services = [
    ...
    },
    {
        ...
        methods: [
            ...
            },
            {
                name: "subscribe",
                type: "subscription",
                signature: "subscribe(request: HostLocalStorageSubscribeRequest): ObservableLike<HostLocalStorageChangeItem, S.CallErrorValue<VersionedHostLocalStorageSubscribeError>>",
                docUrl: "api/local_storage/trait.LocalStorage.html#method.subscribe",
                description: "Subscribe to changes of one key in the product's own storage namespace.\n\nEmits the current value immediately, then one item per later write or\nclear of the key by any of the product's runtimes. A write that leaves\nthe stored bytes unchanged emits nothing.",
                requestDescription: "HostLocalStorageSubscribeRequest",
                exampleSource: 'import { firstValueFrom, from } from "rxjs";\n\nconst item = await firstValueFrom(\n  from(truapi.localStorage.subscribe({ request: { key: "test-key" } })),\n);\nconsole.log("storage change received:", item);',
                requestType: "host-local-storage-subscribe-request",
                responseType: "host-local-storage-change-item",
            },
        ],
    },
    {
        ...
        methods: [
            {
                ...
                requestDescription: "ProductRendererRenderRequest",
                exampleSource: 'truapi.renderer.onRender(({ context, payload }, send) => {\n  send({ tag: "String", value: { text: `${context.tag}: ${payload}` } });\n});',
                requestType: "product-renderer-render-request",
                responseType: "renderer-node",
            },
            {
                name: "action_subscribe",
                type: "subscription",
                ...
                responseType: "host-renderer-action-subscribe-item",
            },
        ],
    },
    {
    ...
    },
    {
        ...
        methods: [
            ...
            {
                name: "sign_raw_unwatermarked_deprecated_with_legacy_account",
                type: "unary",
                signature: "signRawUnwatermarkedDeprecatedWithLegacyAccount(request: HostSignRawWithLegacyAccountRequest): Promise<Result<HostSignPayloadResponse, S.CallErrorValue<VersionedHostSignRawWithLegacyAccountError>>>",
                docUrl: "api/signing/trait.Signing.html#method.sign_raw_unwatermarked_deprecated_with_legacy_account",
                description: "Sign the supplied data without adding or removing a watermark.\n\nTemporary compatibility API for runtime ownership proofs, including the\n32-byte Resources alias used by Humanity. Payload decoding matches\nwatermarked signing, but the decoded bytes are signed exactly as supplied.\nThis permits transaction-shaped data and requires signing authorization\nand explicit user confirmation.\n\n@deprecated Temporary unwatermarked signing; migrate to watermarked signing when the runtime supports it. This API will be removed. See <https://github.com/paritytech/host-rust-core/issues/612>",
                requestDescription: "HostSignRawWithLegacyAccountRequest",
        ...
        methods: [
            {
                ...
                type: "unary",
                signature: "beginOperation(request: HostWorkerBeginOperationRequest): Promise<Result<HostWorkerBeginOperationResponse, S.CallErrorValue<VersionedHostWorkerBeginOperationError>>>",
                docUrl: "api/worker/trait.Worker.html#method.begin_operation",
                description: "Begin a pending operation.",
                requestDescription: "HostWorkerBeginOperationRequest",
                exampleSource: 'const result = await truapi.worker.beginOperation({ label: "funding" });\nassert(result.isOk(), "beginOperation failed:", result);\nconsole.log("operation started:", result.value.id);\nawait truapi.worker.endOperation({ id: result.value.id });',
                requestType: "host-worker-begin-operation-request",
                responseType: "host-worker-begin-operation-response",
            },
            {
                name: "end_operation",
                type: "unary",
                signature: "endOperation(request: HostWorkerEndOperationRequest): Promise<Result<undefined, S.CallErrorValue<VersionedHostWorkerEndOperationError>>>",
                docUrl: "api/worker/trait.Worker.html#method.end_operation",
                description: "End a pending operation. Idempotent: an unknown or already-ended id\nsucceeds, so a retry after an ambiguous failure is safe.",
                requestDescription: "HostWorkerEndOperationRequest",
                exampleSource: 'const begun = await truapi.worker.beginOperation({});\nassert(begun.isOk(), "beginOperation failed:", begun);\nconst result = await truapi.worker.endOperation({ id: begun.value.id });\nassert(result.isOk(), "endOperation failed:", result);\nconsole.log("operation ended");',
                requestType: "host-worker-end-operation-request",
            },
        ],
    },
];
